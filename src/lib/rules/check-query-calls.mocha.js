...
ruleTester.run('check-query-calls', rule, {
  ...
  invalid: [{
    ...
    code: "mongoClient.db.collection('users').find({}, 'test');",
    errors: [{
      message: 'Expected mongoClient.db.collection(\'users\').find call second argument value to be an object or a callback function.',
    }],
  }, {
    code: "mongoClient.db.collection('users').find({}, {}, 'test');",
    errors: [{
      message: 'Expected mongoClient.db.collection(\'users\').find call third argument value to be a callback function.',
    }],
  }, {
    code: "mongoClient.db.collection('users').find({}, {}, function() {}, {});",
    errors: [{
      message: 'Expected mongoClient.db.collection(\'users\').find call to have maximum 3 arguments.',
    }],
  }],
});
