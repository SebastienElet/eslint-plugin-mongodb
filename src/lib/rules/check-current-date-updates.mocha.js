...
ruleTester.run('check-current-date-updates', rule, {
  ...
  invalid: [{
    ...
    settings: {
      mongodb: {
        callPatterns: {
          update: ['^(user|place|session)Collection\\.(updateMany|updateOne)$'],
        },
      },
    },
    errors: [{
    ...
    code: "mongoClient.db.collection('users').updateMany({}, { $currentDate: { $type: 'date',  type: '1664' } });",
    errors: [{
      message: '$currentDate operator should only have a $type modifier (found: type).',
    }],
  }, {
    code: "mongoClient.db.collection('users').updateMany({}, { $currentDate: { $type: '1664' } });",
    errors: [{
