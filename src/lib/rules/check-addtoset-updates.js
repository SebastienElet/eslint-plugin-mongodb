...
function eMQCheckAddToSetUpdates(context) {
  ...
  return utils.lookupCall(context, utils.getCallPatterns('update', context.settings),
    function(callSource, args) {
      ...
      return utils.everyProperties(args[1], [/\$addToSet/], function(property) {
        ...
        return property.value.properties.every(function(propertyNode) {
          ...
          if(utils.nodeIsArray(propertyNode.value)) {
            context.report(
              ...
              ' modifier to add each elements (key: ' + propertyNode.key.name +
              ').'
            );
            return false;
          }
          ...
          return utils.everyProperties(property.value, [/\$each/], function(property) {
