    ...
    (callSource, args) => {
      ...
      return utils.everyProperties(args[1], [/\$currentDate/], property => {
        if('ObjectExpression' !== property.value.type &&
          ...
          context.report(property, `Expected ${property.key.name} operator value to be a boolean or an object.`);
          return false;
        }
        if('ObjectExpression' === property.value.type) {
          return property.value.properties.every(propertyNode => {
            if('$type' !== propertyNode.key.name) {
              context.report(propertyNode, `${property.key.name} operator should only have a $type modifier (found: ${propertyNode.key.name}).`);
              return false;
            }
            if(utils.nodeIsString(propertyNode.value) &&
              -1 === ['timestamp', 'date'].indexOf(propertyNode.value.value)) {
              context.report(propertyNode, `${property.key.name} operator ${propertyNode.key.name} modifier value can be only "date" or "timestamp" (got: ${propertyNode.value.value}).`);
              return false;
            }
            return true;
          });
