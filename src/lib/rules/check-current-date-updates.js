...
function eMQCheckCurrentDateUpdates(context) {
  ...
  return utils.lookupCall(context, utils.getCallPatterns('update', context.settings),
    function(callSource, args) {
      ...
      return utils.everyProperties(args[1], [/\$currentDate/], function(property) {
        ...
        if('ObjectExpression' === property.value.type) {
          return property.value.properties.every(function(propertyNode) {
            ...
            if(utils.nodeIsString(propertyNode.value) &&
              ...
              context.report(propertyNode, property.key.name +
                ' operator ' + propertyNode.key.name + ' modifier value can be' +
                ' only "date" or "timestamp" (got: ' + propertyNode.value.value + ').');
              return false;
            }
            return true;
          });
